...
import { InterfaceLanguageApi, InterfaceMovie } from './interfaces.mjs';
...
// Change page
export function showMovieDetails (id: number): void {
	location.hash = `#movie=${id}`;
}

function goSearchSection (event: Event): void {
...
// Utils
function hiddenAllElements (): void {
...
export function showLikedMovieSection (): void {
	LIKED_MOVIE_SECTION.classList.remove('hidden');

	setLikedMoviesFromLocalStorage();
}

export function amountLikedMovies (): number {
...
async function categoryPage (): Promise<void> {
	const { hash: HASH } = location;
	const ARE_THERE_MOVIES = true;
...
async function moviePage (): Promise<void> {
	const { hash: HASH } = location;
...
function movieDetails (isSkeleton: boolean, movie = {} as InterfaceMovie): void {
...
async function trendsPage (): Promise<void> {
	const ITEM_SKELETON = document.createElement('article');
...
async function updateLanguage () {
	if (CURRENT_LANGUAGE.length === 2) {
		...
		const LOCAL_STORAGE_LANGUAGE: InterfaceLanguageApi = JSON.parse(CURRENT_LANGUAGE);
...
export async function changeLanguage (event: Event) {
